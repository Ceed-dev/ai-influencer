...
- **Created `pipeline/data/scenario.json`**: Extracted 3 sections (hook, body, cta) with scripts and motion reference video URLs
- **Fixed media module endpoints**:
  - `video-generator.js`: `image-to-video` → `motion-control` with `video_url`, `keep_original_sound`, `character_orientation`
...
- **Rewrote orchestrator**: New 3-section loop (Kling → TTS → Lipsync per section) + ffmpeg concat + Drive upload (4 files) + sheet logging
...
### Implemented (Pipeline Phase 0 - 2026-02-10)
- Content generation pipeline: character image → 3-section video → Drive
...
- Orchestrator: 3-section loop with Drive folder creation and sheet logging
- CLI: `node scripts/run-pipeline.js --character-folder <ID> [--dry-run]`
- content_pipeline sheet with new schema (15 columns)
- 21 pipeline tests passing
- Prerequisite for E2E: character image in Drive Characters/ folder

### Pending
- Full E2E test with live fal.ai API calls (need character image in Drive)
