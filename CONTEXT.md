...
## Current System State (as of 2026-02-10, v4.0)
...
### Pending
- Batch execution with `--limit` for multiple videos (Phase 1)
...
pipeline/                   # Node.js content pipeline (v4.0)
  config.js                 # Environment configuration (accounts ID, productionTab added)
  orchestrator.js           # 3-section parallel pipeline (Promise.all, Kling+TTS parallel, extractDriveFileId)
  data/scenario.json        # Scenario template (DEPRECATED - use inventories)
  sheets/client.js          # Google Sheets/Drive API (OAuth2)
  sheets/inventory-reader.js # Inventory reader + ID resolution (v4.0 NEW)
  sheets/production-manager.js # Production tab CRUD (v4.0 NEW)
...
scripts/                    # CLI entry points
  run-pipeline.js           # --video-id <ID> [--limit N] [--dry-run] (v4.0)
...
OPERATIONS.md               # Human operator manual (Japanese, 667 lines)
