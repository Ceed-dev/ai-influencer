...
│  fal.ai: Kling (動画生成, Hook/CTA) + Sync Lipsync (口パク, Hook/CTA) + Fabric 1.0 (Body) │
...
|---|---|---|---|---|---|
| **Kling v2.6 Standard** (fal.ai) | AI動画生成 (Hook/CTA) | 現行パイプライン実績あり | [$0.07/秒](https://fal.ai/models/fal-ai/kling-video/v2.6/standard/motion-control) | $0.35 | [fal.ai Kling](https://fal.ai/models/fal-ai/kling-video/v2.6/standard/motion-control) |
| **Fish Audio TTS** (直接API) | TTS音声生成 | 高品質、低コスト | ~$0.001/セクション | ~$0.001 | [Fish Audio](https://fish.audio) |
| **Sync Lipsync v2 Pro** (fal.ai) | 口パク同期 (Hook/CTA) | 高品質 | [$5.00/分](https://fal.ai/models/fal-ai/sync-lipsync/v2/pro) | $0.42 | [fal.ai Lipsync](https://fal.ai/models/fal-ai/sync-lipsync/v2/pro) |
| **Fabric 1.0** (VEED, fal.ai) | Body リップシンク動画生成 | 1ステップでBody完結 | [$0.15/秒](https://fal.ai/models/veed/fabric-1.0) | $0.75 | [fal.ai Fabric](https://fal.ai/models/veed/fabric-1.0) |
| **ffmpeg** | 3セクション結合 | ローカル処理、無料 | $0 | $0 | [ffmpeg](https://ffmpeg.org/) |
| | | **1動画(3セクション)合計** | | **$2.29 (Pro) / $1.95 (Std)** | [詳細](per-video-cost.md) |
...
|---|---|---|---|---|---|
| **動画制作 (fal.ai)** | fal.ai Kling | 動画生成 (Hook/CTA) | $0.35/セクション × 2 × 1,080本 | **$756** | 5秒×2セクション |
| | fal.ai Sync Lipsync Pro | 口パク同期 (Hook/CTA) | $0.42/セクション × 2 × 1,080本 | **$907** | Pro版使用 |
| | fal.ai Fabric 1.0 | Body動画生成 | $0.75 × 1,080本 | **$810** | 5秒 $0.15/秒 |
| **TTS (Fish Audio)** | Fish Audio TTS | TTS音声生成 | ~$0.001/セクション × 3 × 1,080本 | **$3** | 直接REST API（fal.ai とは別サービス） |
| | **動画制作+TTS小計** | | | **$2,476** | Std版なら$2,109 |
...
| カテゴリ | サービス | 用途 | 単価/根拠 | 月額コスト | 備考 |
|---|---|---|---|---|---|
| **動画制作 (fal.ai)** | fal.ai Kling | 動画生成 (Hook/CTA) | $0.35 × 2 × 3,780 | **$2,646** | |
| | fal.ai Lipsync Pro | 口パク同期 (Hook/CTA) | $0.42 × 2 × 3,780 | **$3,175** | |
| | fal.ai Fabric 1.0 | Body動画生成 | $0.75 × 3,780 | **$2,835** | |
| **TTS (Fish Audio)** | Fish Audio TTS | TTS音声生成 | ~$0.001 × 3 × 3,780 | **$11** | 直接REST API（fal.ai とは別サービス） |
| | **動画制作+TTS小計** | | | **$8,667** | Std: $7,382 |
...
| カテゴリ | サービス | 用途 | 単価/根拠 | 月額コスト | 備考 |
|---|---|---|---|---|---|
| **動画制作 (fal.ai)** | fal.ai Kling | 動画生成 (Hook/CTA) | $0.35 × 2 × 8,280 | **$5,796** | |
| | fal.ai Lipsync Pro | 口パク同期 (Hook/CTA) | $0.42 × 2 × 8,280 | **$6,955** | |
| | fal.ai Fabric 1.0 | Body動画生成 | $0.75 × 8,280 | **$6,210** | |
| **TTS (Fish Audio)** | Fish Audio TTS | TTS音声生成 | ~$0.001 × 3 × 8,280 | **$25** | 直接REST API（fal.ai とは別サービス） |
| | **動画制作+TTS小計** | | | **$18,986** | Std: $16,171 |
...
| カテゴリ | サービス | 用途 | 単価/根拠 | 月額コスト | 備考 |
|---|---|---|---|---|---|
| **動画制作 (fal.ai)** | fal.ai Kling | 動画生成 (Hook/CTA) | $0.35 × 2 × 35,280 | **$24,696** | |
| | fal.ai Lipsync Pro | 口パク同期 (Hook/CTA) | $0.42 × 2 × 35,280 | **$29,635** | → **Std版推奨** |
| | fal.ai Fabric 1.0 | Body動画生成 | $0.75 × 35,280 | **$26,460** | |
| **TTS (Fish Audio)** | Fish Audio TTS | TTS音声生成 | ~$0.001 × 3 × 35,280 | **$106** | 直接REST API（fal.ai とは別サービス） |
| | **動画制作+TTS小計(Pro)** | | | **$80,897** | |
| | **動画制作+TTS小計(Std)** | | Lipsync Std $0.25/セクション × 2 | **$68,902** | **Std版を強く推奨** |
...
| | Cloud Tasks | | | **$1** | |
| **────────** | | | | **────────** | |
| **合計(Pro+OnDemand)** | | | | **$84,890** | |
| **合計(Std+Spot)** | | | | **$72,124** | **推奨構成** |

**並列処理**: 1,176本/日 = 49本/時。12分/本 → 常時9.8本同時。15ワーカーで十分。
**fal.ai同時タスク**: ピーク時 ~40-60。**エンタープライズプラン交渉必須**。
**重要**: 5月からLipsync Standardへの切替を強く推奨（月$12,000のコスト削減、Hook/CTAの2セクションのみ対象）。
...
| カテゴリ | サービス | 用途 | 単価/根拠 | 月額コスト | 備考 |
|---|---|---|---|---|---|
| **動画制作 (fal.ai)** | fal.ai Kling | 動画生成 (Hook/CTA) | $0.35 × 2 × 86,400 | **$60,480** | |
| | fal.ai Lipsync **Std** | 口パク同期 (Hook/CTA) | $0.25 × 2 × 86,400 | **$43,200** | **Std版必須** |
| | fal.ai Fabric 1.0 | Body動画生成 | $0.75 × 86,400 | **$64,800** | |
| **TTS (Fish Audio)** | Fish Audio TTS | TTS音声生成 | ~$0.001 × 3 × 86,400 | **$259** | 直接REST API（fal.ai とは別サービス） |
| | **動画制作+TTS小計(Std)** | | | **$168,739** | Pro版だと$197,856 |
...
> **注意**: GCE Spot VMとCloud Runは代替構成（「又は」）であり、併用前提ではない。上記合計はSpot VM構成。Cloud Run構成の場合はインフラ費+$713（合計$176,058）。

**並列処理の詳細計算**:
- 2,880本/日 ÷ 20時間（4時間バッファ）= 144本/時
- 144本 × 12分/本 ÷ 60分 = **常時28.8本を同時処理**
- 各動画 = 2 Klingタスク(Hook/CTA) + 2 Lipsyncタスク(Hook/CTA) + 1 Fabricタスク(Body) = 計5外部API呼出/動画
- ピーク時fal.ai同時タスク: 28.8 × (2+1) = **~86タスク** → **エンタープライズプラン必須**

**fal.aiエンタープライズ**: 同時100+タスク、ボリュームディスカウント（20-30%削減の可能性）で動画制作費 **$118,000-135,000/月** まで下がる可能性あり。要交渉。
...
```
    ...
    ┌──────┴───────────────┴───────────────┴──────┐
    │              外部API層                        │
    │  fal.ai (Kling / Lipsync / Fabric 1.0)          │
...
| 制約 | 値 | 計算 |
|---|---|---|
| 1動画の制作時間 | ~12分 | Kling(5min) + TTS(5s) + Lipsync(2min) + Fabric(~2min) + ffmpeg(30s) + upload(1min) |
| 順次処理した場合 | 34,560分 = 576時間 | 2,880 × 12分 |
| 利用可能時間 | 20時間/日 | 4時間のバッファ |
| 必要な同時処理数 | **29本** | 2,880 ÷ 20h ÷ (60/12) |
| fal.ai同時タスク | **~86** | 29 × (2 Kling + 1 Fabric) = ~87 ピーク |
...
|---|---|---|---|---|---|---|---|
| **2月** | 50 | 1,080 | $2,109 | $12 | $417 | $123 | **$2,661** |
| **3月** | 160 | 3,780 | $7,382 | $39 | $417 | $255 | **$8,093** |
| **4月** | 340 | 8,280 | $16,171 | $83 | $424 | $607 | **$17,285** |
| **5月** | 1,480 | 35,280 | $68,902 | $352 | $948 | $970 | **$71,172** |
| **6月** | 3,500 | 86,400 | $168,739 | $820 | $978 | $2,475 | **$173,012** |
| **────** | | | | | | | **────────** |
| **5ヶ月累計** | | 134,820 | $263,303 | $1,306 | $3,184 | $4,430 | **$272,223** |

### コスト構成比（6月）

| カテゴリ | 月額 | 構成比 |
|---|---|---|
| 動画制作（fal.ai）+ TTS（Fish Audio） | $168,739 | **97.5%** |
| AI（OpenAI） | $820 | 0.5% |
| 投稿/分析 | $978 | 0.6% |
| インフラ | $2,475 | 1.4% |

> **動画制作APIが総コストの97.5%を占める**。fal.aiとのエンタープライズ交渉でのボリュームディスカウント（20-30%）が最大のコスト削減レバー。

### コスト削減オプション

| 施策 | 効果 | トレードオフ |
|---|---|---|
| **Lipsync Pro→Std** | -17%（6月で-$29,376/月、Hook/CTAの2セクションのみ） | 口パク品質がやや低下 |
| **Kling v1.6 Std** ($0.045/秒) | -36%（Hook/CTA動画生成コスト） | v2.6より画質低下 |
| **fal.aiエンタープライズ** | -20〜30%（推定） | 要交渉・最低契約あり |
| **LatentSync** ($0.20/動画) | Hook/CTA Lipsync費を-70% | fal.ai経由、品質要検証 |
| **Fish Audio TTS**（独立サービス） | TTS費-90%（移行済み） | ElevenLabsから移行完了。fal.ai とは別の直接REST API |
| **Spot VM** | インフラ費-54% | 中断リスクあり（リトライで対応） |

> **注**: BodyセクションはFabric 1.0を使用するため、Lipsyncティア選択（Pro/Std）やKlingバージョン変更の影響を受けない。コスト削減はHook/CTAの2セクションにのみ適用される。

### 最大削減時の見積もり（6月）

| 項目 | 標準構成 | 最大削減構成 | 削減額 |
|---|---|---|---|
| 動画生成（Kling v1.6 Std, Hook/CTA） | $60,480 | $38,880 | -$21,600 |
| Fabric 1.0（Body） | $64,800 | $64,800 | $0（削減対象外） |
| TTS（Fish Audio） | $259 | $259 | $0（移行済み） |
| Lipsync（LatentSync, Hook/CTA） | $43,200 | $11,520 | -$31,680 |
| AI | $820 | $820 | $0 |
| 投稿/分析 | $978 | $978 | $0 |
| インフラ（Spot） | $2,475 | $2,475 | $0 |
| **合計** | **$173,012** | **$119,732** | **-$53,280 (-31%)** |
...
| fal.ai Fabric 1.0 (VEED) | https://fal.ai/models/veed/fabric-1.0 |
