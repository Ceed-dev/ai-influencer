...
- `POST /api/auth/youtube/initiate` — Google OAuth URL生成 + CSRF nonce設定（CSRFクッキー有効期限300秒）
- `GET /api/auth/youtube/callback` — OAuth code→token交換 → channels.list でチャンネル情報取得 → accounts upsert → `/auth/youtube/result` リダイレクト
- `GET /api/demo/youtube/channel` — チャンネル情報取得（`youtube.upload` スコープ内で channels.list 呼び出し）。snippet + statistics (subscriberCount, videoCount, viewCount)
