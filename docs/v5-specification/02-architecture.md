...
├────────────────────────────────┼────────────────────────────────────────┤
│                                ▼                                        │
│   Layer 3: MCP Server (自作 Node.js, ~73ツール)                         │
│                                                                         │
│   ┌────────────────────────────────────────────────────────────────┐    │
...
また、v4.0では固定3セクション (Hook/Body/CTA) だったが、v5.0ではセクション数・種類をコンテンツごとに **動的に決定** する。ツールスペシャリストがコンテンツ種別に応じた最適なセクション分割を推奨する。
...
```
          ...
          └────────┬───────┘
                   ...
                   ▼
          ┌────────────────────────────────────────────────┐
          │ ツールスペシャリスト — レシピ + セクション構成  │
          │                                                │
          │  ・コンテンツ種別 (動画/テキスト) の判定       │
          │  ・ジャンル・キャラクター特性の分析            │
          │  ・セクション構成の決定                        │
          │    (3セクション固定ではなく、コンテンツに最適な│
          │     分割をレシピとして定義)                    │
          │    例: ショート動画 → intro(2s) + main(8s)     │
          │        + cta(3s) の3分割                        │
          │    例: 長尺解説 → hook(5s) + point1(15s)       │
          │        + point2(15s) + summary(5s) の4分割     │
          │    例: テキスト → セクション分割なし           │
          │ ・最適なツール組み合わせ (レシピ) をDB参照+生成│
          │  ・過去の成功/失敗パターンに基づく推奨         │
          │                                                │
          │  MCPツール:                                    │
          │   get_tool_recipes                             │
...
| セクション構成 | 固定3分割 (Hook/Body/CTA) | 動的N分割 (コンテンツ種別に応じてツールスペシャリストが決定) |
...
**ステータス管理の2層構造**:
- `content.status`: 制作ライフサイクル (`planned` → `producing` → `ready`)
...
| テーブル | ステータス遷移 | 担当グラフ |
|---|---|---|
| `content` | `planned` → `producing` → `ready` → `analyzed` | 戦略→制作→(投稿+計測完了後)→戦略 |
...
| `mcp-server` | `node:20-slim` | MCP Server (~73ツール) | 3100 | SQL実行 + Drive API |
