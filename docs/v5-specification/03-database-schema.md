...
>
> **テーブル数**: 34テーブル (Entity 3 / Production 3 / Intelligence 12 / Operations 4 / Observability 5 / Tool Management 6 / System Management 1)
>
...
- [6. Tool Management Tables (ツール管理テーブル)](#6-tool-management-tables-ツール管理テーブル)
  ...
  - [6.6 content_playbooks — コンテンツ制作Playbook](#66-content_playbooks--コンテンツ制作playbook)
...
- [12. 想定クエリパターン](#12-想定クエリパターン)
  ...
  - [12.9 Planner: Playbookセマンティック検索](#129-planner-playbookセマンティック検索)
...
| **Tool Management** | 6 | AIツールの知識管理・制作レシピ・プロンプト改善・Playbook | tool_catalog, tool_experiences, tool_external_sources, production_recipes, prompt_suggestions, content_playbooks |
...
erDiagram
    ...
    content_playbooks {
        INTEGER id PK
        TEXT playbook_name UK "Playbook名 (UNIQUE)"
        TEXT content_type "コンテンツ種別"
        TEXT content_format "short_video/text_post/image_post"
        TEXT niche "ニッチカテゴリ"
        TEXT platform "対象プラットフォーム"
        TEXT markdown_content "Playbookの全文 (Markdown)"
        VECTOR embedding "vector(1536)"
        NUMERIC avg_effectiveness_score "有効性スコア(平均)"
        INTEGER times_used "使用回数"
    }
...
### 3.6 content_learnings — コンテンツ単位マイクロサイクル学習
...
### 6.6 content_playbooks — コンテンツ制作Playbook

チームメイトがClaudeとの試行錯誤で得たコンテンツ制作ノウハウ（Playbook）を構造化して管理する。制作フロー・ツールの使い方・パラメータ設定・ハマりどころ等をMarkdownで記録し、pgvectorのembeddingにより類似Playbookのセマンティック検索を実現する。Planner Agentがコンテンツ計画策定時に参照し、制作品質の安定化・自動化を支援する。
...
CREATE TABLE content_playbooks (
    ...
    playbook_name           TEXT NOT NULL,
        -- Playbook名 (一意)
        -- 例: 'ai_beauty_real_short', 'tech_explainer_tiktok', 'pet_reaction_reel'
        -- content_type + platform等を組み合わせた分かりやすい名前
    content_type            TEXT NOT NULL,
        -- コンテンツ種別
        -- 例: 'short_video_ai_influencer', 'product_review', 'tutorial'
    content_format          TEXT NOT NULL CHECK (content_format IN ('short_video', 'text_post', 'image_post')),
        -- コンテンツフォーマット (content.content_format / production_recipes.content_format と一致)
        -- short_video: 短尺動画 (YouTube Shorts, TikTok, IG Reels)
        -- text_post: テキスト投稿 (X/Twitter)
        -- image_post: 画像投稿 (将来拡張)
    niche                   TEXT,
        -- ニッチカテゴリ
        -- 例: 'beauty', 'fitness', 'lifehack', 'tech'
        -- NULLの場合: ニッチ横断の汎用Playbook
    platform                TEXT,
        -- 主な対象プラットフォーム
        -- youtube / tiktok / instagram / x / NULL (全プラットフォーム共通)
    ...
    markdown_content        TEXT NOT NULL,
        -- PlaybookのMarkdown全文
        -- 構成: メタデータ / コンテンツ概要 / 参考リンク / 制作フロー /
        --       ツールの特性・癖 / フックの型 / クオリティ基準 / コスト目安 / 学んだこと
        -- Planner Agentが読んでそのまま制作指示に使えるレベルの詳細さ

    -- ベクトル検索
    embedding               vector(1536),
        -- markdown_contentのベクトル埋め込み (OpenAI text-embedding-3-small)
        -- 用途: Planner AgentがコンテンツのnicheやformatでPlaybookをセマンティック検索

    -- 有効性追跡
    avg_effectiveness_score NUMERIC(4,2) CHECK (avg_effectiveness_score BETWEEN 0 AND 1),
        -- Playbookの有効性スコア 0.00〜1.00 (累積平均)
        -- 制作完了後の品質スコアをローリング平均で更新
        -- NULLの場合: まだ使用実績なし
    times_used              INTEGER NOT NULL DEFAULT 0,
        -- このPlaybookが参照された回数
        -- Planner Agentがget_playbookで取得するたびにインクリメント

    -- メタデータ
    created_by              TEXT NOT NULL DEFAULT 'human' CHECK (created_by IN ('human', 'agent')),
        -- 作成者種別: 'human'(チームメイト) / 'agent'(Tool Specialist等)

    -- ステータス
    is_active               BOOLEAN NOT NULL DEFAULT true,
        -- このPlaybookが現在利用可能か
        -- falseの場合: 非推奨、廃止、改訂中等
...
| Instagram | `ig_user_id`, `page_id`, `oauth.long_lived_token`, `oauth.ig_user_id`, `oauth.expires_at` | Facebook OAuth経由。long_lived_tokenは60日間有効。app_id/app_secretはsystem_settings(INSTAGRAM_APP_ID/INSTAGRAM_APP_SECRET)に保存 |
...
--   }
-- }
-- Note: app_id/app_secret は system_settings (INSTAGRAM_APP_ID / INSTAGRAM_APP_SECRET) に保存
--
-- X (Twitter):
-- {
--   "user_id": "xxx",
...
CREATE INDEX idx_content_learnings_embedding ON content_learnings
    USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);
    -- マイクロサイクル学習のベクトル検索 (search_content_learnings)
    -- 注意: 10,000件超でIVFFlatへの移行を検討
CREATE INDEX idx_content_learnings_niche ON content_learnings(niche);
    -- ニッチ別フィルタ
CREATE INDEX idx_content_learnings_verdict ON content_learnings(micro_verdict);
...
CREATE INDEX idx_content_playbooks_embedding ON content_playbooks USING HNSW (embedding vector_cosine_ops);
    -- HNSW (Hierarchical Navigable Small World) インデックス
    -- Planner AgentがnicheやformatでPlaybookをセマンティック検索
...
### 12.9 Planner: Playbookセマンティック検索

```sql
-- MCPツール: search_playbooks (CPB-002)
-- Planner Agentがコンテンツ計画策定時に、niche/formatに合ったPlaybookをpgvectorで検索
-- パラメータ: $1=query_embedding(query_textから生成), $2=content_format, $3=similarity_threshold(0.7), $4=limit(5)
SELECT cp.id, cp.playbook_name, cp.content_format, cp.niche,
       cp.platform, cp.avg_effectiveness_score, cp.times_used,
       1 - (cp.embedding <=> $1) AS similarity
FROM content_playbooks cp
WHERE cp.is_active = true
  AND cp.content_format = $2
  AND cp.embedding IS NOT NULL
  AND 1 - (cp.embedding <=> $1) >= $3
ORDER BY cp.embedding <=> $1
LIMIT $4;

-- MCPツール: get_playbook (CPB-003)
-- Planner AgentがPlaybookの全文を取得し、制作指示に活用
-- パラメータ: $1=id
SELECT cp.id, cp.playbook_name, cp.content_format, cp.niche,
       cp.platform, cp.markdown_content,
       cp.avg_effectiveness_score, cp.times_used,
...
-- times_used をインクリメント
UPDATE content_playbooks SET times_used = times_used + 1 WHERE id = $1;
```
