...
```mermaid
flowchart TB
    subgraph L1["Layer 1: 戦略エージェント (社長) x 1体"]
        S["Claude Opus 4.6<br/>全アカウントのKPI監視 / 大方針決定 / リソース配分<br/>トリガー: 日次 (毎朝)"]
    end

    subgraph L2["Layer 2: 専門職エージェント x 4〜5体"]
        R["リサーチャー (Researcher)<br/>Claude Sonnet 4.5<br/>市場調査・トレンド収集<br/>トリガー: 数時間ごと<br/>x 1〜数体"]
        AN["アナリスト (Analyst)<br/>Claude Sonnet 4.5<br/>パフォーマンス分析<br/>仮説検証・知見蓄積<br/>トリガー: 計測完了後<br/>x 1体"]
        TS["ツールスペシャリスト (Tool Specialist)<br/>Claude Sonnet 4.5<br/>AIツール特性把握 / 最適ツール組み合わせ提案 / 制作レシピ設計<br/>トリガー: 制作計画策定時 + 定期知識更新<br/>x 1体"]
        DC["データキュレーター (Data Curator)<br/>Claude Sonnet 4.5<br/>生データの構造化 / コンポーネント自動生成 / キャラクター自動生成 / 重複チェック・品質判定<br/>トリガー: 連続実行 (キューポーリング)<br/>x 1体"]
    end

    subgraph L3["Layer 3: プランナーエージェント (部長) x N体"]
        P["Claude Sonnet 4.5<br/>ニッチ/クラスター別に20〜50アカウント担当<br/>コンテンツ計画・仮説立案・数日先の投稿計画<br/>トリガー: 日次<br/>スケール方針: プランナーを増やすだけで水平スケール"]
    end

    subgraph L4["Layer 4: ワーカーエージェント (作業員) — ステートレス・プール"]
        W1["動画制作ワーカー<br/>(コード)<br/>動画生成<br/>ツールSP推奨に従う<br/>負荷で増減"]
        W2["テキスト制作ワーカー<br/>(コード)<br/>X投稿文生成<br/>キャラ設定+シナリオ活用<br/>負荷で増減"]
        W3["投稿ワーカー<br/>(コード)<br/>各プラットフォームへ投稿<br/>負荷で増減"]
        W4["計測ワーカー<br/>(コード)<br/>メトリクス収集<br/>負荷で増減"]
    end
...
```mermaid
flowchart TB
    Boss["社長 (戦略Agent)<br/>Claude Opus 4.6"]
    Spec["専門職<br/>リサーチャー / アナリスト /<br/>ツールスペシャリスト<br/>Sonnet"]
    Plan["プランナー (部長)<br/>Sonnet x N"]
    Work["ワーカー (作業員) — コード<br/>動画制作 / テキスト制作 / 投稿 / 計測"]
    DB["PostgreSQL (全データの最終保存先)"]

    Boss -- "(1) 下向き: 方針指示" --> Plan
    Boss -- "(1) 下向き: 方針指示" --> Spec
    Spec -- "(2) 上向き: 分析報告" --> Boss
    Spec -- "(3) 横方向: 判断材料・ツール推奨" --> Plan
    Plan -- "(1) 下向き: 制作指示" --> Work
    Work -- "(2) 上向き: 完了報告" --> Plan
    Work --> DB
```
...
```mermaid
sequenceDiagram
    participant Boss as 社長 (Opus)
    participant PlanA as プランナーA (Sonnet)
    participant PlanB as プランナーB (Sonnet)
    participant Analyst as アナリスト (Sonnet)
    participant ToolSP as ツールスペシャリスト (Sonnet)

    Boss->>PlanA: 今サイクルの方針: beautyニッチに注力、<br/>朝投稿を全アカウントの50%に適用
    PlanA-->>Boss: 方針を受領。beauty 20アカウントの<br/>コンテンツ計画を策定します
    Boss->>PlanB: (同上の方針指示)
    PlanB-->>Boss: 方針を受領。tech 30アカウントの<br/>計画を策定します
    Analyst->>Boss: 前サイクルの分析報告: 仮説的中率62%、<br/>朝投稿テスト群のengagement +35%
    ToolSP->>Boss: 本日の制作にはKling + Fish Audio + Sync Lipsyncの<br/>デフォルトレシピを推奨。西洋人キャラはRunway + Hedraを推奨

    Note over Boss,ToolSP: 通信媒体: DBのステータス変更 + MCPツール経由のデータ<br/>頻度: 低い (サイクル単位 = 日次)<br/>各エージェント = System Prompt + MCP Tools + Schedule
```
...
```mermaid
flowchart TB
    Planner["プランナー (LLM)<br/>content INSERT, status='planned'<br/>task_queue INSERT, type='produce'"]

    subgraph TQ["task_queue テーブル"]
        T1["id=1 type=produce status=queued pri=0"]
        T2["id=2 type=produce status=queued pri=0"]
        T3["id=3 type=produce status=queued pri=10"]
        T4["id=4 type=publish status=queued pri=0"]
        T5["id=5 type=measure status=queued pri=0"]
    end

    ProdW["制作ワーカー (ポーリング)<br/>SELECT .. WHERE type='produce'<br/>AND status='queued'<br/>ORDER BY pri DESC LIMIT 1<br/>FOR UPDATE SKIP LOCKED"]
    PubW["投稿ワーカー (ポーリング)<br/>SELECT .. WHERE type='publish'<br/>AND status='queued'<br/>LIMIT 1 FOR UPDATE SKIP LOCKED"]
...
> **通信媒体**: task_queue テーブル (PostgreSQL) / **頻度**: 高い (30秒ポーリング) / **並行制御**: SELECT FOR UPDATE SKIP LOCKED
...
```mermaid
flowchart TB
    START["START<br/>(cronトリガー)"]
    CI["collect_intel<br/>リサーチャー (Sonnet)<br/>最新の市場データを収集・整理"]
    AC["analyze_cycle<br/>アナリスト (Sonnet)<br/>前サイクルの分析<br/>仮説検証・知見抽出"]
    SS["set_strategy<br/>社長 (Opus)<br/>サイクル方針決定<br/>リソース配分・人間指示の処理"]
    PC["plan_content<br/>プランナー x N (Sonnet)<br/>仮説立案・コンテンツ計画<br/>スケジュール設定<br/>※差戻し時はfeedback参照"]
    ST["select_tools<br/>ツールスペシャリスト (Sonnet)<br/>制作レシピ設計<br/>各コンテンツに最適ツール割当"]
    AP["approve_plan<br/>社長 (Opus)<br/>計画の承認/差戻し<br/>コスト上限確認"]
    RA["reflect_all<br/>全エージェント (各自のLLM)<br/>セルフリフレクション実行<br/>個別学習記録・人間への報告"]
    END_["END"]
...
```mermaid
flowchart TB
    START["START<br/>(PM2常駐プロセス)"]
    PT["poll_tasks<br/>タスクキューから制作タスクを取得<br/>MCP: get_production_task"]
    SL["sleep (30sec)"]
    FD["fetch_data<br/>キャラクター情報・コンポーネントデータを取得<br/>MCP: get_character_info, get_component_data (xN)<br/>update_content_status → status='producing'"]
    DP["dispatch<br/>content_format で分岐"]

    subgraph VID["generate_video (動画制作ワーカー)"]
        direction TB
        VR["recipe_id → production_recipes.steps 取得<br/>レシピに基づきツールを選択して実行"]
        VP["Promise.all (Nセクション並列)<br/>各セクション: [動画Gen] + [TTS] → [Lipsync]<br/>※ ツール名はレシピで指定<br/>※ セクション数はcontent_sectionsで定義"]
        VC["ffmpeg concat + blackdetect → Google Drive保存"]
        VR --> VP --> VC
    end

    GT["generate_text (テキスト制作ワーカー)<br/>LLM (Sonnet) によるテキスト生成<br/>入力: シナリオ, characters.personality, プラットフォーム制約<br/>出力: content_sections.script, 自己スコアリング<br/>MCP: update_content_status"]
    IP["image_post<br/>(将来拡張)"]

    QC["quality_check<br/>動画: ファイルサイズ, 黒フレーム検出, Drive保存確認<br/>テキスト: 文字数, キャラ一貫性<br/>合格: ready / 不合格: failed"]
    HE["handle_error<br/>error_message記録, retry_count確認<br/>リトライ可能 → task再キュー<br/>不可 → failed"]
...
flowchart TB
    SC1["戦略サイクルグラフ (日次)"]
    DB["PostgreSQL<br/><br/>content.status:<br/>pending_approval → planned → producing → ready → analyzed<br/>(error / cancelled = 終端ステータス)<br/><br/>publications.status:<br/>scheduled → posted → measured<br/><br/>task_queue: produce / publish / measure"]
    PP["制作パイプライングラフ (連続)"]
    PS["投稿スケジューラーグラフ (連続)"]
    MJ["計測ジョブグラフ (連続)"]
    SC2["戦略サイクルグラフ (次の日次)"]

    SC1 -- "content INSERT (status='pending_approval' or 'planned')<br/>task_queue INSERT (type='produce')" --> DB
    DB -- "content.status: planned → producing" --> PP
    DB -- "publications.status: scheduled → posted" --> PS
    DB -- "publications.status: posted → measured" --> MJ
    DB -- "metrics, analyses, learnings<br/>→ 次の戦略サイクルで読み取り" --> SC2
```
...
```mermaid
flowchart TB
    A["投稿前: 予測スナップショット生成 (G5)<br/>baseline x (1 + Σ(weight_i x adjustment_i))"]
    B["48h後: 単発分析<br/>predicted vs actual → micro_verdict<br/>→ content_learnings に即時記録"]
    C["7d後: 累積分析 + 予測誤差算出<br/>prediction_error_7d = |predicted - actual| / actual<br/>→ 5テーブルpgvector検索 + AI解釈<br/>→ content_learnings.cumulative_context に記録"]
    D["tier別バッチ: ウェイト再計算<br/>Error Correlation方式 → 9要素のweight自動調整"]
    E["次の投稿で改善された予測値"]
...
| メトリクス数 | 予測精度 | 状態 |
|-------------|---------|------|
| 0-500 | ~50% | コールドスタート、全adj=0 |
| 500-5,000 | ~70% | 補正係数が効き始める |
| 5,000-50,000 | ~85% | ウェイト日次調整で収束 |
| 50,000+ | ~92% | 12h更新で微調整、上限値 |
...
```mermaid
flowchart TB
    subgraph MACRO["マクロサイクル (日次)"]
        M1["戦略方針<br/>社長(Opus)"] --> M2["リソース配分<br/>社長(Opus)"] --> M3["承認<br/>社長"] --> M4["日次リフレクション<br/>全エージェント"]
    end

    subgraph MICRO["マイクロサイクル (per-content x N件/日)"]
        C1["Content 1: 知見検索 → 計画 → 制作 → 投稿 → 計測 →<br/>マイクロ分析 → マイクロ反省 → 学習記録"]
        C2["Content 2: 知見検索 ← Content 1の学習を含む → 計画 → ..."]
        C3["Content 3: 知見検索 ← Content 1+2の学習を含む → 計画 → ..."]
        CN["Content N: 知見検索 ← Content 1〜N-1の全学習を含む → ..."]
        C1 --> C2 --> C3 -.-> CN
    end
...
> 各コンテンツの学習が即座に次のコンテンツに反映される連鎖。1日3,000件 = 3,000回の改善ループ。

**コンテンツ1件あたりの学習ループ (マイクロサイクル)**:

```mermaid
flowchart TB
    subgraph PhaseA["Phase A: コンテンツ計画時"]
        A1["1. 過去の類似コンテンツの学習をベクトル検索 (Type B分析)"]
        A2["2. 検索結果をコンテンツ計画に反映"]
        A3["3. 仮説を立てて制作 → 投稿"]
        A1 --> A2 --> A3
    end

    subgraph PhaseB["Phase B: メトリクス到着時 (投稿48h後)"]
        B1["4. THIS コンテンツの predicted vs actual を比較 (Type A分析)"]
        B2["5. マイクロ反省: 何が効いた/何が効かなかったかを30秒で分析"]
        B3["6. 学習記録: embedding付きで保存 → 即座に次のコンテンツから検索可能"]
        B1 --> B2 --> B3
    end
...
> **Type A分析**: このコンテンツ固有の学び (即時学習) / **Type B分析**: 全履歴データからの類似パターン学習 (ベクトル検索)

### 7.1 全体フロー図

```mermaid
flowchart LR
    subgraph MacroStart["マクロサイクル (日次)"]
        S1["Step 1<br/>戦略Agent<br/>データ確認<br/>(集計結果)"]
        S2["Step 2<br/>戦略Agent<br/>方針決定"]
        S3["Step 3<br/>プランナー<br/>仮説立案<br/>(知見検索)"]
        S4["Step 4<br/>プランナー<br/>コンテンツ計画"]
        S45["Step 4.5<br/>ツールSP<br/>ツール選択<br/>レシピ設計"]
        S1 --> S2 --> S3 --> S4 --> S45
    end

    subgraph MicroCycle["マイクロサイクル (per-content x N件)"]
        S5["Step 5<br/>制作ワーカー<br/>コンテンツ生成"]
        S6["Step 6<br/>投稿ワーカー<br/>投稿実行"]
        S7["Step 7<br/>計測ワーカー<br/>メトリクス収集"]
        S8m["Step 8m<br/>マイクロ分析<br/>(per-content)"]
        S9m["Step 9m<br/>マイクロ反省<br/>(per-content)"]
        S10m["Step 10m<br/>マイクロ学習記録<br/>(即時)"]
        S5 --> S6 --> S7 --> S8m --> S9m --> S10m
    end

    subgraph MacroEnd["マクロサイクル終了 (日次)"]
        S11M["Step 11M<br/>日次集計<br/>+ 知見昇格"]
        S12M["Step 12M<br/>日次リフレクション<br/>+ 構造的改善"]
        S11M --> S12M
    end

    MacroStart --> MicroCycle --> MacroEnd
```
...
```mermaid
flowchart TB
    OBS["① 観察: ダッシュボードでエージェントの思考ログを確認"]
    ID["② 問題の特定<br/>例: リサーチャーの調査範囲が広すぎる。<br/>ニッチ別に深く掘るべきなのに、浅く広い情報しか取れていない"]
    EDIT["③ プロンプト編集<br/>researcher.md の思考アプローチセクションを修正:<br/>Before: 各ニッチのトレンドを幅広く調査してください<br/>After: 最も注力すべきニッチに絞り、競合3〜5件の<br/>直近10投稿を深掘り分析。狭く深い調査を優先"]
    APPLY["④ 反映 (次サイクルから自動適用)"]
    MEAS["⑤ 効果測定: Before/After比較<br/>Before: relevance_score平均 0.45<br/>After: relevance_score平均 0.72<br/>結論: 調査品質が向上。この変更を維持する"]

    OBS --> ID --> EDIT --> APPLY --> MEAS --> OBS
```

> **思考ログ例** (戦略サイクルグラフの出力): [社長] KPI確認完了。全体view数は前週比-5% / [社長] beautyニッチが好調 (+12%), tech低迷 / [社長] 方針: beautyに70%リソース配分 / [プランナーA] 仮説: 朝投稿でview +20% / [プランナーA] 根拠: market_intel #234 / [リサーチャー] 調査対象: 美容トレンド全般
...
```mermaid
flowchart LR
    LRR["load_recent_reflections<br/>前回の振り返り +<br/>本日のマイクロ分析集計を読み込み<br/>前回の改善点を今回のタスクに適用"]
    LLD["load_learning_directives<br/>人間からの学習方法指導を確認<br/>リフレクション方法に反映する"]
    MT["main_task<br/>今回の仕事を自己評価"]
    RF["reflect<br/>マイクロ分析の集計結果を<br/>踏まえた構造的改善に集中"]
    SR["save_reflection"]
    END_["end"]

    LRR --> LLD --> MT --> RF --> SR --> END_
    SR -.-> |"継続改善ループ<br/>次マクロサイクルへ"| LRR
```

> **マイクロサイクルとの関係**: マイクロリフレクション (Step 9m) = コンテンツ1件の即時振り返り → content_learningsに保存 / マクロリフレクション (本セクション) = 1日の構造的振り返り → agent_reflectionsに保存 / マクロリフレクション時、get_daily_micro_analyses_summary() でマイクロ分析の集計を参照
...
```mermaid
flowchart TB
    subgraph L1["Layer 1: 共有知見 (learningsテーブル) — 会社のWiki"]
        L1D["全エージェントが参照。統計的に有意な知見のみ<br/>confidence >= LEARNING_CONFIDENCE_THRESHOLD (0.7)<br/>マイクロサイクルで繰り返し確認された知見が昇格"]
    end

    subgraph L2["Layer 2: コンテンツ学習 (content_learningsテーブル) — 作業ログ"]
        L2D["コンテンツ1件ごとのマイクロサイクル学習 (§7 Step 8m-10m)<br/>embedding付きでベクトル検索可能 — 次のコンテンツ計画で即時参照<br/>3,000+件/日の学習が蓄積 → 最大のデータソース<br/>predicted vs actual, contributing_factors, detractors"]
    end

    subgraph L3["Layer 3: 個別学習メモリ (agent_individual_learningsテーブル)"]
        N1["社長のノート<br/>自分だけが書く<br/>主に自分が読む<br/>(他人も覗ける)"]
        N2["リサーチャーのノート<br/>自分だけが書く<br/>主に自分が読む<br/>(他人も覗ける)"]
        N3["アナリストのノート<br/>自分だけが書く<br/>主に自分が読む<br/>(他人も覗ける)"]
        N4["プランナーのノート<br/>自分だけが書く<br/>主に自分が読む<br/>(他人も覗ける)"]
    end
...
```mermaid
flowchart LR
    subgraph BEFORE["従来の通信 (セクション2.2) — 一方向: 人間→AI"]
        H1["人間"] -- "human_directives" --> B1["社長"]
        B1 --> P1["プランナー"]
    end

    subgraph AFTER["本セクションで追加 — 双方向: AI→人間→AI"]
        Boss2["社長"]
        Res2["リサーチャー"]
        Ana2["アナリスト"]
        Tool2["ツールスペシャリスト"]
        ...
        Boss2 -- "agent_communications" --> H2
        Res2 -- "agent_communications" --> H2
        Ana2 -- "agent_communications" --> H2
        Tool2 -- "agent_communications" --> H2
        Data2 -- "agent_communications" --> H2
        Plan2 -- "agent_communications" --> H2
        H2 -- "human_directives (返信)" --> Boss2
    end
```
...
```mermaid
sequenceDiagram
    participant Agent as エージェント (例: リサーチャー)
    participant DB as PostgreSQL
    participant Dash as ダッシュボード (人間側)

    Agent->>Agent: タスク実行中 → 困りごとを検出
    Agent->>DB: submit_agent_message({<br/>agent_type: "researcher",<br/>message_type: "struggle",<br/>content: "TikTokデータが3日前のもの...",<br/>priority: "normal"})
    Note over DB: agent_communications INSERT
    DB->>Dash: 受信トレイに表示<br/>[高] ACC_0015急落報告<br/>[普] TikTokデータ問題<br/>[低] 週次レポート
    Dash->>Dash: 人間が返信を入力
    Dash->>DB: submit_human_directive({<br/>directive_type: "instruction",<br/>content: "TikTok APIを追加検討中。<br/>当面はGoogle Trendsで代替を"})
    Note over DB: human_directives INSERT
    Agent->>DB: 次サイクル開始時:<br/>get_human_responses({agent_type: "researcher"})
    DB->>Agent: 返信内容を取得
    Agent->>Agent: 了解しました。Google Trendsでの代替を継続します。<br/>→ タスクに反映
```
...
```mermaid
flowchart LR
    subgraph LOGIC["送信判断ロジック (shouldNotifyHuman関数)"]
        direction TB
        S1["自己評価が3サイクル連続で低下<br/>(self_score が 3回連続で前回以下)"] --> T1["struggle"]
        S2["同じto_improveが3サイクル以上続いている<br/>(構造的問題の可能性)"] --> T1
        S3["異常値を検出 (KPIの急落/急伸)<br/>(normal値の2σ以上の乖離)"] --> T1
        S4["目標KPIを3サイクル連続達成<br/>(成功パターンの横展開提案)"] --> T2["proposal"]
        S5["新しい知見の信頼度が0.85超え<br/>(高信頼知見の活用拡大提案)"] --> T2
        S6["自分では判断できない二者択一<br/>(リスクが高い意思決定)"] --> T3["question"]
        S7["週次/日次の定期タイミング<br/>(社長: 日次、その他: 週次)"] --> T4["status_report"]
    end
```
...
```mermaid
flowchart TB
    subgraph SRC1["情報源1: 外部情報の定期収集"]
        S1["AIツール関連のXアカウント投稿 (開発者、パワーユーザー)<br/>各ツール公式ドキュメントの変更<br/>プレスリリース、ブログ記事<br/>リサーチャーからのツール関連市場情報<br/><br/>頻度: 日次 (リサーチャーと連携)<br/>保存先: tool_catalog テーブル + MCPツール経由"]
    end

    subgraph SRC2["情報源2: 制作経験の蓄積"]
        S2["制作結果の品質評価 (品質スコア + 人間フィードバック)<br/>どのツール組み合わせが良かったか / 悪かったか<br/>失敗パターンの記録 (エラー内容、回避策)<br/><br/>頻度: 制作完了ごと<br/>保存先: tool_experiences テーブル + 個別学習メモリ"]
    end

    subgraph SRC3["情報源3: 人間による経験則の教育"]
        S3["ダッシュボードから tool_experiences テーブルへ経験則追加<br/>例: アジア人の顔はKlingが自然、西洋人はRunwayが自然<br/>例: Fish Audio + Sync Lipsyncは日本語の口パク精度が高い<br/>手動Claude作業で得た知見のDB登録<br/><br/>頻度: 必要時 (人間判断)<br/>保存先: tool_experiences テーブル (DB管理)"]
    end
...
```mermaid
flowchart TB
    P1["Phase 1: 情報収集<br/>外部情報スキャン<br/>(リサーチャー連携)"]
    P2["Phase 2: 制作レシピ設計<br/>最適ツール選択<br/>レシピ生成"]
    P3["Phase 3: 結果評価<br/>品質スコア記録<br/>成功/失敗パターン記録"]
    P4["Phase 4: 知識更新<br/>ツール知識DB更新<br/>個別学習メモリ更新<br/>推奨マトリックス更新"]
...
```mermaid
flowchart LR
    S1["Step 1: 知見の整理<br/>知見を構造化して整理"]
    S2["Step 2: カテゴリ分け<br/>ツール特性 / コンテンツ戦略 /<br/>プラットフォーム特性 / その他"]
    S3["Step 3: プロンプト反映<br/>対応するDBテーブルに反映"]
    S4["Step 4: バージョン記録<br/>agent_prompt_versionsテーブルに<br/>変更を記録"]
    S5["Step 5: 効果測定<br/>変更前後のパフォーマンス比較"]
    S6["Step 6: 共有知見登録<br/>有効な知見をlearningsテーブルにも登録<br/>(全エージェント共有)"]
...
flowchart TB
    ...
    subgraph MICRO["マイクロサイクル (per-content, ~30秒, イベント駆動)"]
        M8["Step 8m: create_micro_analysis → content_learnings"]
        M9["Step 9m: save_micro_reflection → content_learnings"]
        M10["Step 10m: 共有知見昇格判定 → learnings UPDATE"]
        CLDB["content_learnings DB (embedding付き)<br/>即座に次のコンテンツのStep 3でベクトル検索可能"]
        M8 --> CLDB
        M9 --> CLDB
        M10 --> CLDB
    end

    subgraph MACRO["マクロサイクル (日次集計)"]
        MA1["daily_micro_aggregation → analyses"]
        MA2["algorithm_performance記録"]
        MA3["agent_reflections (構造的改善)"]
    end

    KDB["知見DB<br/>(learnings + content_learnings + agent_individual_learnings)"]

    subgraph AGENTS["各エージェントの次コンテンツ/次サイクル"]
        AG1["プランナー: search_content_learnings → 仮説立案に反映"]
        AG2["戦略Agent: 日次集計結果を方針に反映"]
        AG3["ツールSP: ツール選択に経験を反映"]
        AG4["データキュレーター: 品質基準を更新"]
    end
