...
|-------|------|------|------|------|-----------|
| 1 | データ基盤 | 2/16 - 3/7 | 3週 | 全データの一元管理基盤を構築 | PostgreSQL + pgvector + MCP Server + データ移行完了 |
| 2 | ワーカー層 | 3/9 - 3/28 | 3週 | DB駆動のパイプラインを稼働させる | 制作・投稿・計測ワーカーがDB起点で動作 |
| 3 | インテリジェンス層 | 3/30 - 4/25 | 4週 | AIが自律的に市場調査・分析・学習する | リサーチャー + アナリスト + 仮説検証 + pgvector検索 |
| 4 | 戦略・計画層 | 4/27 - 5/23 | 4週 | 全自動サイクルを完成させる | 戦略Agent + プランナー + 全プラットフォーム対応 |
| 5 | ダッシュボード + スケール | 5/25 - 6/27 | 5週 | 人間の監視UI + 3,500アカウント対応 | Next.js Dashboard + プランナー増設 + 本番運用 |
...
**目的**: 全データをPostgreSQL + pgvectorに一元管理し、MCP Serverで外部からアクセス可能にする

**v4.0との関係**: v4.0パイプラインは変更せず稼働継続。Sheetsデータを読み取ってPostgreSQLにコピーする（Sheetsは引き続き正のソース）

#### Week 1 (2/16 - 2/21): PostgreSQLセットアップ + Entityテーブル

| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | GCE上にPostgreSQL 16+インストール、初期設定 | PostgreSQL稼働、接続確認 |
...
| 木 | `cycles` + `human_directives` + `task_queue` + `algorithm_performance` + `agent_prompt_versions` + `agent_thought_logs` | Operations 4テーブル + エージェント観察2テーブル |
| 金 | 全トリガー作成 (`updated_at` 自動更新) + 全インデックス確認 | 17テーブル完成 |
...
**Phase 1 完了時点のシステム状態**:
```
PostgreSQL 16+ (GCE)         MCP Server (Node.js)
┌─────────────────────┐     ┌─────────────────────┐
│ 17テーブル            │     │ ~33ツール             │
│ (含: prompt_versions │     │ 基本CRUD              │
│  + thought_logs)     │◄───│ テスト済み             │
│ 全インデックス         │     │                      │
│ 既存データ移行済み     │     │                      │
└─────────────────────┘     └─────────────────────┘
...
| 火 | 制作ワーカーの骨格 (LangGraphノードとして定義) | タスク取得 → orchestrator.js呼出 → 結果DB保存 |
| 水 | `inventory-reader.js` → MCP Server への読み取り元切替 | DB起点でシナリオ・キャラ・モーション取得 |
| 木 | `production-manager.js` → PostgreSQL への書き込み先切替 | ステータス更新・メタデータ保存がDB直接 |
| 金 | 制作ワーカーのdry-runテスト | `--dry-run` でDB→制作→DB保存の一連動作確認 |
| 土 | 制作ワーカーの本番E2Eテスト (fal.ai実行) | 1本の動画がDB起点で制作完了 |

**エージェントプロンプト基盤** (Week 4 並行作業):
- `prompts/` ディレクトリ作成、各エージェント種別 (制作WK, 投稿WK, 計測WK) のプロンプトファイル (`.md`) を外部化
...
#### Week 6 (3/23 - 3/28): 計測ワーカー + E2E完了

| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | YouTube Analytics APIアダプター実装 | views, likes, comments等の取得 |
| 火 | 計測ワーカーの骨格 (LangGraphノード) | `measure_after`到来 → API取得 → `metrics`保存 |
| 水 | engagement_rate等の計算ロジック実装 (MCP Server内) | 計測値の自動集計 |
| 木 | 計測ワーカーのテスト | Week 5で投稿した動画の計測成功 |
| 金 | DB駆動パイプラインのE2Eテスト | planned → producing → ready → posted → measured |
...
**人間チューニング方針**: Phase 3はエージェントの「知性」が形成される最重要フェーズ。各LangGraphノードにthought logging（思考過程の記録）を組み込み、`prompts/*.md` からプロンプトを動的に読み込む仕組みを導入する。日次〜週次でプロンプトチューニングを実施し、エージェントの推論品質を継続的に改善する運用が期待される。

#### Week 7 (3/30 - 4/4): LangGraph.js + リサーチャー基盤

| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | LangGraph.js v1.0 セットアップ + TypeScript設定 | プロジェクト統合、基本グラフ実行確認 |
...
| 金 | WebSearch統合 (トレンドデータ収集) | 市場データのDB自動保存 |
| 土 | リサーチャーの動作テスト | `market_intel`テーブルにデータ蓄積開始 |

#### Week 8 (4/6 - 4/11): リサーチャー完成 + MCP追加ツール
...
| 木 | MCP分析管理ツール (5ツール) 実装 | `get_performance_summary`, `get_trending_topics`, etc. |
| 金 | MCP戦略管理ツール (4ツール) + 設定管理ツール (3ツール) | `review_plan`, `approve_plan`, etc. |
| 土 | 全MCPツールのテスト (~60ツール完成) | 全ツール動作確認 |

#### Week 9 (4/13 - 4/18): アナリストエージェント
...
| 金 | 異常検知ロジック (views急落・急増の自動検出) | `anomaly_detection` 分析 |
| 土 | アナリストの結合テスト | メトリクス → 分析 → 知見の一連動作確認 |

#### Week 10 (4/20 - 4/25): Embedding + pgvector検索

| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | Embeddingモデル統合 (text-embedding-3-small or Voyage-3) | テキスト → vector(1536) 変換パイプライン |
| 火 | 仮説のembedding自動生成 + HNSW検索テスト | 類似仮説の自動検索 |
| 水 | 知見のembedding + 市場情報のembedding | 3テーブルのベクトル検索対応完了 |
| 木 | 重複仮説防止ロジック (コサイン類似度閾値) | 類似度0.9以上の仮説を自動マージ |
| 金 | `algorithm_performance` テーブルへの自動記録 | 仮説的中率・予測誤差の追跡開始 |
| 土 | インテリジェンス層の結合テスト | リサーチャー → アナリスト → 知見蓄積のE2E |
...
**目的**: 戦略Agent + プランナーによる全自動サイクルを構築し、人間の介入なしでコンテンツ企画→制作→投稿→計測→分析→学習が回るようにする
...
| 木 | 計画承認/差戻しフロー (approve_plan / reject_plan) | Supervisorパターンの承認ループ |
...
→ 全自動でコンテンツ企画→制作→投稿→計測→分析→学習
→ 4プラットフォーム対応 (YouTube, TikTok, Instagram, X)
→ 人間は介入不要 (ただし監視UIはまだなし)
```
...
#### Week 15 (5/25 - 5/30): ダッシュボード基盤

| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | Next.js + Shadcn/ui プロジェクト初期化 | ダッシュボード基盤 |
...
#### Week 16 (6/1 - 6/6): 人間介入パネル + 追加ページ

| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | 人間介入パネル: 仮説投入フォーム + プロンプトエディタUI (`prompts/*.md` のブラウザ上編集・保存・版管理) | `human_directives` 書込み + プロンプトのダッシュボード編集 |
| 火 | 人間介入パネル: 参考コンテンツ指定 + 設定変更 + 人間-エージェント対話インターフェース (1対1のフィードバック送受信) | 投稿頻度・計測タイミング変更UI + エージェントへの直接フィードバック |
| 水 | 学習ログページ (知見一覧、仮説→結果、改善提案) + thought logビューア (`agent_thought_logs` の検索・フィルタ・時系列表示) | 知見の可視化 + エージェント思考過程の閲覧 |
| 木 | エージェント実行ログページ (タスク状況、エラー率) + エージェント進化ダッシュボード (プロンプト版ごとの精度比較) | 運用監視UI + プロンプト改善効果の可視化 |
| 金 | 認証・アクセス制御の実装 | ログイン機能 |
| 土 | ダッシュボード全機能の結合テスト | 全ページ + 書込み機能の動作確認 |

#### Week 17 (6/8 - 6/13): スケール対応 (1)

| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | プランナー増設 (3-5クラスター) | ニッチ別のプランナー並列稼働 |
| 火 | タスクキューの並列処理最適化 | 複数ワーカーの同時実行 |
| 水 | PostgreSQLクエリ最適化 (EXPLAIN ANALYZE) | スロークエリの解消 |
| 木 | コネクションプール調整 (pgBouncer検討) | DB接続の安定化 |
| 金 | 負荷テスト (500アカウント相当のシミュレーション) | ボトルネック特定 |
| 土 | 負荷テスト結果に基づく改善 | スループット改善 |

#### Week 18 (6/15 - 6/20): スケール対応 (2) + 安定化

| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | プランナー増設 (10-15クラスター、各200-350アカウント担当) | 3,500アカウント対応のプランナー群 |
| 火 | fal.aiクレジット管理の自動化 (残高監視 + アラート) | コスト管理 |
| 水 | アカウント一括登録スクリプト | 大量アカウントの効率的なセットアップ |
| 木 | 全プラットフォームのOAuth一括設定ツール | アカウント設定の自動化 |
| 金 | 1,000アカウント規模の負荷テスト | 大規模運用の安定性確認 |
| 土 | 監視・アラートの設定 (PM2 + ログ監視) | 障害検知体制 |
...
| 土 | GASコード・Sheetsの完全停止 + ドキュメント更新 | **v5.0本番運用完了** |

## 4. マイルストーン

| ID | マイルストーン | 予定日 | 判定基準 | KPI対応 |
|----|-------------|--------|---------|---------|
| M1 | DB稼働 | 3/7 (W3土) | PostgreSQL 17テーブル + MCP Server ~33ツールが動作。既存データ移行完了 | 50アカウント管理可能 |
| M2 | DB駆動パイプライン完成 | 3/28 (W6土) | DB起点で planned→producing→ready→posted→measured の全ステータス遷移が動作 | 160アカウントのDB管理開始 |
| M3 | AI学習開始 | 4/25 (W10土) | リサーチャー + アナリストが稼働。仮説生成 → 検証 → 知見蓄積が自動で回る | 340アカウントの分析開始 |
| M4 | 全自動サイクル | 5/23 (W14土) | 戦略Agent → プランナー → ワーカーの完全自律サイクル。4プラットフォーム対応 | 1,480アカウントの自動運用 |
| M5 | 本番運用開始 | 6/27 (W19土) | ダッシュボード稼働。3,500アカウントのフルスケール運用 | 3,500アカウント完全稼働 |

### マイルストーン判定チェックリスト

**M1: DB稼働**
- [ ] PostgreSQL 16+がGCE上で稼働
- [ ] pgvector拡張が有効
- [ ] 17テーブル全て作成済み (`agent_prompt_versions` + `agent_thought_logs` 含む)
- [ ] 全インデックス作成済み
- [ ] `updated_at`トリガー動作確認
- [ ] 既存Sheetsデータの移行完了
...
- [ ] 24時間の無人稼働テスト成功
- [ ] `prompts/` ディレクトリにワーカー用プロンプトファイル配置済み
- [ ] thought logging共通ロガーが動作し `agent_thought_logs` に記録

**M3: AI学習開始**
- [ ] LangGraph.jsグラフが正常実行
...
```
Phase 1内:
  PostgreSQL設定 → スキーマ作成 → データ移行 → MCP Server
  (W1)            (W2)          (W2土)       (W3)

Phase 2内:
  タスクディスパッチャー → 制作WK → 投稿WK → 計測WK
  (W4月)                 (W4)    (W5)     (W6)

Phase 3内:
  LangGraph設定 → リサーチャー → アナリスト → Embedding
  (W7月-火)      (W7-8)        (W9)        (W10)
...
|-------|----------|---------|--------|---------------------|------|
| 1 | pgvectorの構築トラブル、データ移行の型変換エラー | 中 | 中 | Week 3の作業がWeek 4にずれ込む場合 | pgvectorはDockerイメージでの代替構築を用意 |
...
| PostgreSQL | GCE上にインストール | 将来的にCloud SQL移行の可能性あり |
| PM2 | 既存PM2設定を拡張 | LangGraphプロセスも管理 |
