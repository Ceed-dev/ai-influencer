...
| 木 | `cycles` + `human_directives` + `task_queue` + `algorithm_performance` | Operations 4テーブル |
| 金 | 全トリガー作成 (`updated_at` 自動更新) + 全インデックス確認 | 15テーブル完成 |
...
| 火 | `langchain-mcp-adapters` による MCP Server接続 | LangGraphノードからMCPツール呼び出し成功 |
| 水 | チェックポイント設定 (PostgreSQL or ファイルシステム) | 耐久実行の基盤 |
| 木 | リサーチャーエージェントの骨格 (LangGraphグラフ) | ノード定義 + ステート管理 |
...
| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | アナリストエージェントの骨格 (LangGraphグラフ) | ノード定義 + ステート管理 |
...
| 火 | 戦略Agent (Opus) のプロンプト設計 + ノード実装 | KPI監視 + 方針決定ロジック |
...
| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | プランナーエージェントの骨格 (最初は1クラスター) | アカウント × シナリオのマッチング |
...
| 木 | アルゴリズム精度推移ページ (仮説的中率、学習曲線) | 精度トレンドの可視化 |
...
| 日 | 作業内容 | 成果物 |
|----|---------|--------|
| 月 | 人間介入パネル: 仮説投入フォーム | `human_directives` への書込みUI |
| 火 | 人間介入パネル: 参考コンテンツ指定 + 設定変更 | 投稿頻度・計測タイミング変更UI |
| 水 | 学習ログページ (知見一覧、仮説→結果、改善提案) | 知見の可視化 |
| 木 | エージェント実行ログページ (タスク状況、エラー率) | 運用監視UI |
...
|----|-------------|--------|---------|---------|
| M1 | DB稼働 | 3/7 (W3土) | PostgreSQL 15テーブル + MCP Server ~33ツールが動作。既存データ移行完了 | 50アカウント管理可能 |
