...
- [14. 数式クイックリファレンス](#14-数式クイックリファレンス)
...
> **KPI定義**: v5.0のKPIは**プラットフォーム別の平均インプレッション数**である（アカウント数ではない）。目標値: TikTok 15,000 / IG 10,000 / YT 20,000 / X 10,000。KPI達成率 = min(1.0, avg_impressions / target)。

| 指標 | Month 1（3月） | Month 3（5月） | Month 6（6月〜8月） | Month 12（12月） | 上限 |
|---|---|---|---|---|---|
| 仮説的中率 | 25〜35% | 45〜60% | 70〜82% | 88〜93% | **92%** |
| 予測誤差 | 45〜65% | 20〜35% | 10〜20% | 5〜10% | 5% |
| 蓄積知見数 | 30〜60 | 350〜700 | 800〜2,400 | 4,000+ | - |
| コンテンツ品質改善率 | 基準値 | +20〜40% | +40〜70% | +80〜150% | - |
| 月間学習イベント数 | 900〜2,400 | 9,000〜24,000 | 15,000〜60,000 | 105,000 | - |

> **精度上限 92%の根拠**: v5.0の上限は**92%**（楽観シナリオで95%、悲観で70%）。残る8%は制御不能な外部変数（プラットフォームアルゴリズム変更、バイラル偶発性、季節変動）に起因する。日次サイクル型の旧設計（天井65〜70%）と比較して、per-content学習モデルにより+22〜27%の天井引き上げを実現する。
>
...
4. **天井は92%（楽観95%）**: 1日3,500件以上のデータポイントにより、ノイズを統計的に排除可能。残る8%は予測不能な外部イベント（バイラル偶発性、プラットフォームアルゴリズム変更、季節変動）
5. **pgvectorによるクロスニッチ学習が精度80%突破の鍵**: ニッチAでの知見がニッチBに転用されることで、学習速度が2〜3倍に加速し、個別ニッチでは到達できない精度レベルに到達する
6. **per-content学習で1,000〜3,000倍の学習速度**: 日次サイクル型（月間10〜15イベント）に対し、per-contentモデルは月間15,000〜45,000イベント。即座の分析（コンテンツ完了→48h以内にメトリクス→verdict判定）+ ベクトル検索による歴史的パターンマッチングの二重構造で実現
7. **エージェント個別の精度追跡が全体最適化を駆動する**: 各エージェント（リサーチャー、アナリスト、プランナー、戦略エージェント）が自身の精度を自己評価・自己改善し、個々の改善が連鎖的にシステム全体の精度を押し上げる好循環を形成する
8. **新規アカウントは300〜1,000インプレッションからスタート**: 各プラットフォームの新規アカウントの現実的な初期値から、学習ループによりKPI目標（TikTok 15,000等）に向けて成長させる。KPI = プラットフォーム別平均インプレッション数
...
| 月 | 運用月 | Phase | 月間コンテンツ数 | 仮説数(累計) | 的中率(予測) | 予測誤差 | 蓄積知見数 | 備考 |
|---|---|---|---|---|---|---|---|---|
| 3月 | Month 1 | 初期テスト | 900〜2,400 | 50〜150 | **25〜35%** | 45〜65% | 30〜60 | DB稼働開始。コンテンツ単位学習で初月から大量データ蓄積 |
| 4月 | Month 2 | 学習開始 | 3,000〜6,000 | 200〜500 | 35〜50% | 25〜40% | 120〜250 | LangGraphエージェント初稼働。per-content分析で高速パターン認識 |
| 5月 | Month 3 | 自律運用 | 9,000〜24,000 | 600〜1,500 | **45〜60%** | 18〜30% | 350〜700 | 全エージェント稼働。クロスニッチ学習が本格化 |
| 6月 | Month 4 | 最適化 | 15,000〜45,000 | 1,500〜4,000 | 60〜75% | 12〜22% | 800〜1,500 | ダッシュボード + 人間介入。累計5万件以上の学習イベントで精度急上昇 |
| 8月 | Month 6 | 成長前期 | 45,000〜75,000 | 3,500〜7,000 | **70〜82%** | 8〜15% | 1,800〜2,800 | 全9補正係数活性化。weight再計算がtier3に移行 |
| 12月 | Month 10 | 成熟期 | 105,000 | 15,000+ | **88〜93%** | 5〜10% | 4,000+ | 天井付近（上限92%）。予測不能な外部イベントのみが誤差源 |

> **精度成長の軌跡**: Month 1: 25-35% → Month 3: 45-60% → Month 6: 70-82% → Month 12: 88-93%。上限は**92%**（中央値予測）。旧日次サイクル型の天井65-70%を大幅に超過する。
...
> **日次サイクル型との比較**: 日次サイクルでは月間10〜15サイクル（＝学習イベント）だった。per-contentモデルでは6月時点で月間15,000〜45,000イベントとなり、**1,000〜3,000倍**の学習速度を実現する。この圧倒的なデータ量が精度上限92%（旧設計65-70%から+22-27pt向上）の基盤となる。
>
> **導出式**: `倍率 = per-content月間イベント数 / 日次サイクル月間イベント数`
> - 下限: 15,000 ÷ 15 = **1,000倍**
> - 上限: 45,000 ÷ 15 = **3,000倍**
> - (日次サイクル月間10〜15イベント = 1日1サイクル × 稼働日数。per-contentは上表Phase 5の値)
...
### 3.4 的中率92%（上限）が到達可能な理由

per-content学習モデルにより、従来の日次サイクル型（天井65〜70%）を大幅に超える精度が実現可能になる。中央値予測では**92%**、楽観シナリオで95%、悲観シナリオで70%:
...
> **92%到達の必要条件**: ①月間1,000件以上のコンテンツ制作（Phase 4以降で達成）、②5ニッチ以上の並列運用、③市場リサーチの2〜4時間更新、④3ヶ月以上の累積学習データ、⑤計測パイプラインの収集率95%以上
...
```mermaid
flowchart TB
    A["コンテンツ制作（900〜2,400件/月）"] --> B["各コンテンツの即時分析\n（投稿後48h以内にメトリクス収集→verdict判定）"]
    B --> C["仮説: 一般的なSNSマーケティング知識\n+ リサーチャーの最新データ + 即時フィードバック"]
    C --> D["結果: 25〜35%が的中\n（大量データにより初月からパターンの兆候を検出）"]
    D --> E["知見: 「このニッチでは〇〇が重要」\n（中確信度 — 数百件のデータに基づく）"]
```
...
```mermaid
flowchart TB
    A["コンテンツ制作（3,000〜6,000件/月、累計6,000〜12,000件）"] --> B["仮説: 蓄積知見（120〜250件）\n+ ニッチ固有パターン + リサーチデータ\n+ ベクトル類似検索"]
    B --> C["結果: 40〜55%が的中\n（統計的に裏付けられたパターンに基づく仮説の精度が急上昇）"]
    C --> D["知見: 高確信度のアクション可能な知見\n（「フック3秒以内に質問形式 = 離脱率-30%」\nevidence_count: 50+）"]
```
...
```mermaid
flowchart TB
    A["コンテンツ制作（9,000〜45,000件/月、累計30,000〜80,000件）"] --> B["仮説: 高確信度の蓄積知見\n+ クロスニッチの類似パターン\n+ 時系列トレンド + 二重分析"]
    B --> C["結果: 55〜82%が的中\n（複合仮説 + 大量データの統計的パワーにより精度が70%を突破）"]
    C --> D["知見: 複合的な高精度戦略\n（「美容ニッチ + Before/After + 15秒\n+ 朝7時投稿 = 完視聴率78%±5%」）"]
```
...
```mermaid
flowchart TB
    A["コンテンツ制作（60,000〜105,000件/月、累計200,000件以上）"] --> B["仮説: 精緻な予測モデル\n+ 外部変数の即時組み込み\n+ リアルタイムトレンド検知"]
    B --> C["結果: 82〜93%が的中\n（残る誤差は予測不能な外部イベントのみ）"]
    C --> D["知見: 防御的知見\n（「アルゴリズム変更の兆候を日間データから数時間で検知」）\n+ 精密予測モデル"]
```
...
的中率(%)
  ...
  ※ 上限92% (中央値予測)。旧日次サイクル型の天井65-70%から+22-27pt向上
```
...
```mermaid
flowchart TB
    subgraph components["コンテンツ品質スコア"]
        H["フック\nScore: 8"]
        B["本編\nScore: 6"]
        C["CTA\nScore: 7"]
        V["音声\nScore: 9"]
    end
    H --> AVG["加重平均スコア: 7.2/10"]
    B --> AVG
    C --> AVG
    V --> AVG
    AVG --> HIGH["Score >= 7:\n積極活用 (replicate)"]
...
```mermaid
flowchart TB
    A["美容ニッチの知見:\n「Before/After形式のフック + 15秒 + 朝7時投稿\n→ 完視聴率78%」"]
    B["pgvector類似検索:\nフィットネスニッチとの類似度 = 0.82"]
    C["フィットネスニッチに適用:\n「Before/After形式のフック + 15秒 + 朝7時投稿\n→ 完視聴率???」"]
    D["結果: 完視聴率72% → confirmed\n（類似度の高さに比例して的中率が高い）"]
    A --> B --> C --> D
```
...
```mermaid
flowchart TB
    R["リサーチャー\n調査精度 向上"] -->|"より質の高い市場データ"| AN["アナリスト\n分析品質 向上"]
    AN -->|"より正確な知見・仮説検証"| PL["プランナー\n企画成功率 向上"]
    PL -->|"より高品質なコンテンツ"| CT["コンテンツ成果\nメトリクス向上"]
    CT -->|"より豊富で正確なパフォーマンスデータ"| ST["戦略エージェント\n戦略的中率 向上"]
    ST -->|"より的確なサイクル方針・リソース配分\n（調査方針の精緻化・重点領域の指示）"| R
...
> 1つのエージェントの改善が、次のエージェントの入力品質を向上させ、サイクル全体の精度が加速度的に改善される
...
精度(%)
  ...
  per-content + 自己反省: 12ヶ月で92% (上限92%, 楽観95%)
...
精度の天井(%) — per-contentモデル
  ...
  80 |  │  ████████████████████████████████  プランナー              │
     ...
     ※ per-contentモデルにより日次サイクル型比+22〜27ptの天井引き上げ (65-70% → 92%)
```
...
```mermaid
flowchart TB
    subgraph researcher["リサーチャーの天井到達後"]
        R1["アナリストの「どの分析が仮説confirmedに\nつながったか」を参照"]
        R2["「confirmedにつながりやすい情報カテゴリ」を\n逆算して収集対象を最適化"]
        R3["調査精度がさらに+3〜5%向上"]
        R1 --> R2 --> R3
    end
    subgraph planner["プランナーの天井到達後"]
        P1["リサーチャーの「トレンド変動タイミング」\nデータを直接参照"]
        P2["トレンドの立ち上がり期にコンテンツを\n投入する精度が向上"]
        P3["企画成功率がさらに+2〜4%向上"]
        P1 --> P2 --> P3
    end
```
...
│                      v5.0 12ヶ月シミュレーション                   │
├─────┬────────┬──────┬──────┬──────┬──────┬──────┬───────────────┤
│ 月  │ 的中率  │ 誤差  │ 知見数 │ 品質  │ アカ数 │ 平均Imp│ 総Imp(千)    │
├─────┼────────┼──────┼──────┼──────┼──────┼──────┼───────────────┤
│ 3月 │ 30%    │ 55%  │ 45   │ 4.8  │ 75   │ 500  │ 1,125         │
│ 4月 │ 48%    │ 32%  │ 180  │ 5.8  │ 150  │ 800  │ 3,600         │
│ 5月 │ 63%    │ 20%  │ 520  │ 6.5  │ 550  │ 1,500│ 24,750        │
│ 6月 │ 76%    │ 15%  │ 1,150│ 7.2  │ 1,000│ 2,500│ 75,000        │
│ 7月 │ 80%    │ 12%  │ 1,800│ 7.6  │ 1,400│ 3,500│ 147,000       │
│ 8月 │ 84%    │ 10%  │ 2,400│ 8.0  │ 1,800│ 4,200│ 226,800       │
│ 9月 │ 87%    │ 8%   │ 3,000│ 8.3  │ 2,200│ 5,000│ 330,000       │
│10月 │ 89%    │ 7%   │ 3,500│ 8.5  │ 2,600│ 5,700│ 442,000       │
│11月 │ 91%    │ 6%   │ 3,800│ 8.7  │ 3,000│ 6,300│ 570,000       │
│12月 │ 92%    │ 6%   │ 4,200│ 8.8  │ 3,500│ 7,000│ 735,000       │
├─────┴────────┴──────┴──────┴──────┴──────┴──────┴───────────────┤
│ 的中率: confirmed/(confirmed+rejected)                          │
│ 誤差: avg(|predicted - actual| / actual)                        │
│ 品質: 10点満点の加重平均スコア                                      │
│ 平均Imp: 1アカウントあたりの平均インプレッション/投稿 (**KPI指標**)  │
│ 総Imp: 平均Imp × アカウント数 × 30投稿/月                         │
│                                                                  │
│ ※ KPI達成率 = min(1.0, 平均Imp / KPI目標)                         │
│   KPI目標: TikTok=15,000 / IG=10,000 / YT=20,000 / X=10,000    │
│   12月の平均Imp 7,000 → 全体KPI達成率 ~47-70% (プラットフォーム別)│
│   ※ 平均Impは全アカウント混合値。成熟アカウントはKPI目標に到達済み  │
└──────────────────────────────────────────────────────────────────┘
```
...
### 10.6 新規アカウントのインプレッション成長曲線

新規アカウントは各プラットフォームで300〜1,000インプレッションからスタートする。v5.0の学習ループにより、以下の成長曲線でKPI目標に向かって成長する。

#### プラットフォーム別初期値と目標

| プラットフォーム | 新規アカウント初期値 | KPI目標 | 必要成長倍率 | 目標到達予測月 |
|---|---|---|---|---|
| TikTok | 300〜1,000 imp | 15,000 imp | 15〜50x | 4〜6ヶ月目 |
| Instagram | 300〜800 imp | 10,000 imp | 12〜33x | 4〜6ヶ月目 |
| YouTube Shorts | 500〜1,000 imp | 20,000 imp | 20〜40x | 5〜8ヶ月目 |
| X (Twitter) | 200〜500 imp | 10,000 imp | 20〜50x | 5〜7ヶ月目 |

#### 学習による成長メカニズム

```mermaid
flowchart TB
    INIT["初期: 300-1,000 imp"] --> PA1

    subgraph PA["Phase A: テストプール突破 (1-2ヶ月目)"]
        PA1["学習: コホートベースラインで予測\n(source='cohort')"]
        PA2["効果: 完視聴率改善\n→ アルゴリズム評価上昇"]
        PA3["結果: 1,000-3,000 imp\n(3-10x成長)"]
        PA1 --> PA2 --> PA3
    end

    PA3 --> PB1

    subgraph PB["Phase B: パターン最適化 (3-4ヶ月目)"]
        PB1["学習: own_historyベースライン有効化\n+ 4-7補正係数活性化"]
        PB2["効果: 投稿時間・フック形式\n・ハッシュタグの最適化"]
        PB3["結果: 3,000-8,000 imp\n(3-8x追加成長)"]
        PB1 --> PB2 --> PB3
    end

    PB3 --> PC1

    subgraph PC["Phase C: 精密運用 (5-6ヶ月目)"]
        PC1["学習: 全9補正係数活性化\n+ クロスニッチ知見転用"]
        PC2["効果: コンテンツ品質スコア7+\n+ 最適投稿戦略"]
        PC3["結果: 8,000-15,000 imp\n(KPI目標到達圏)"]
        PC1 --> PC2 --> PC3
    end

    PC3 --> PD1

    subgraph PD["Phase D: 高精度維持 (7ヶ月目以降)"]
        PD1["学習: 精緻な予測モデル\n+ 外部変数即時対応"]
        PD2["効果: 安定的にKPI目標を\n達成・超過"]
        PD3["結果: 15,000-30,000 imp\n(目標1-2x)"]
        PD1 --> PD2 --> PD3
    end
```

#### 月別インプレッション成長予測（TikTokの例）

| 月 | 平均imp/投稿 | 成長ドライバー | 仮説的中率 |
|---|---|---|---|
| 1ヶ月目 | 500〜1,200 | LLM一般知識 + 初期A/Bテスト | 25〜35% |
| 2ヶ月目 | 1,000〜2,500 | 初期パターン認識 + 投稿時間最適化 | 35〜45% |
| 3ヶ月目 | 2,000〜5,000 | ニッチ固有パターン確立 + フック最適化 | 45〜60% |
| 4ヶ月目 | 4,000〜8,000 | 全補正係数活性化 + クロスニッチ学習開始 | 55〜70% |
| 5ヶ月目 | 7,000〜12,000 | 品質スコア7+到達 + weight最適化進行 | 65〜78% |
| 6ヶ月目 | 10,000〜18,000 | **KPI目標15,000到達圏** + 精密予測 | 70〜82% |

> **成長の複利効果**: インプレッションが増加 → メトリクスデータが豊富に → 学習精度が向上 → さらに高品質なコンテンツ → さらにインプレッション増加。この正のフィードバックループにより、成長は線形ではなく指数的に加速する。ただし、各プラットフォームのアルゴリズム評価には閾値があり（テストプール→レコメンド→For You等）、閾値を突破するまでは成長が緩やかで、突破後に急加速するS字曲線パターンとなる。
...
本セクションは、本ドキュメント全体の主要数式を一覧化したクイックリファレンスである。詳細は各参照先セクションを参照。

| # | 数式名 | 数式 | 参照先 |
|---|---|---|---|
| F1 | **インプレッション予測** | `predicted = baseline x (1 + SUM(weight_i x adjustment_i))` | section 15.1 |
| F2 | **KPI達成率** | `min(1.0, avg_impressions / kpi_target)` | section 23.1 |
| F3 | **予測精度** | `GREATEST(0, 1.0 - ABS(predicted - actual) / actual)` | section 23.3 |
| F4 | **仮説的中率** | `confirmed_count / (confirmed_count + rejected_count)` | section 12.1 |
| F5 | **予測誤差** | `AVG(\|predicted[key] - actual[key]\| / NULLIF(actual[key], 0))` | section 12.2 |
| F6 | **verdict判定** | `error <= 0.3 -> confirmed, error >= 0.5 -> rejected, else inconclusive` | section 12.3 |
| F7 | **品質スコア** | `SUM(weight_i x min(1.0, actual / platform_niche_median)) x 10` | section 12.5 |
| F8 | **confidence更新（confirmed）** | `min(0.95, old + (1 - old) x 0.10)` | section 12.6 |
| F9 | **confidence更新（rejected）** | `max(0.05, old - 0.15)` | section 12.6 |
| F10 | **confidence更新（inconclusive）** | `old + 0.02` | section 12.6 |
| F11 | **weight再計算（EMA）** | `alpha x calculated + (1 - alpha) x old` (alpha=0.3) | section 18.1 |
...
> **per-content vs 日次サイクルの比較**:
>
> | 項目 | 日次サイクル型（旧設計） | per-content型（v5.0） | 改善倍率 |
> |---|---|---|---|
> | 学習トリガー | 1日1回の定期バッチ | 各コンテンツの計測完了時 | イベント駆動 |
> | 月間学習イベント数 | 10〜15 | 15,000〜45,000 (Phase 5) | **1,000〜3,000x** |
> | フィードバック遅延 | 数日〜数週間 | ~30秒（マイクロサイクル） | 即時 |
> | 精度上限 | 65〜70% | **92%** | +22〜27pt |
> | 導出式 | - | 15,000 / 15 = 1,000x〜45,000 / 15 = 3,000x | - |
...
> **重要**: v5.0のKPIは**プラットフォーム別の平均インプレッション数**である（アカウント数ではない）。新規アカウントは300〜1,000インプレッションから始まり、学習ループにより以下の目標値に向けて成長する。KPI達成率はこの「平均インプレッション数が目標に対してどの程度達成しているか」を測定する。

### 23.1 KPI達成率の定義

```
KPI達成率 = min(1.0, avg_impressions / kpi_target)
```

- `avg_impressions`: 対象アカウント群の月間下旬（21日〜月末）の平均インプレッション数（7日計測値）
- `kpi_target`: プラットフォーム別目標値（下表）
- 目標超過は**1.0（100%）でキャップ**
- プラットフォームごとに算出

| プラットフォーム | KPI目標（system_settings） | 新規アカウント初期値 | 成長倍率 |
|---|---|---|---|
| TikTok | 15,000 (`KPI_TARGET_TIKTOK`) | 300〜1,000 | 15〜50x |
| Instagram | 10,000 (`KPI_TARGET_INSTAGRAM`) | 300〜800 | 12〜33x |
| YouTube | 20,000 (`KPI_TARGET_YOUTUBE`) | 500〜1,000 | 20〜40x |
| X (Twitter) | 10,000 (`KPI_TARGET_TWITTER`) | 200〜500 | 20〜50x |
...
> **注**: 本SQLは [07-kpi-analysis.md](07-kpi-analysis.md) セクション10.6と同一仕様。KPI目標値は `system_settings` テーブルから動的取得する（ハードコーディング禁止）。
...
WITH platform_kpi AS (
  ...
  WHERE a.status = 'active'
    ...
    AND p.posted_at >= DATE_TRUNC('month', $year_month::DATE) + INTERVAL '20 days'
...
- **根拠計算**:
  - baseline精度 ~55% (own_history確立)
  - 補正による改善 +10-15% (4-7 active factors × avg impact)
  - 結果: 45-60% prediction accuracy
...
┌──────────────────────────────────────────────────────────────────┐
│                                                                  │
│   月     3月   4月   5月   6月   8月   12月                       │
│  運用月  M1    M2    M3    M4    M6    M10                        │
│          │     │     │     │     │     │                         │
│  的中率  30%   43%   53%   68%   76%   92%    ← 上限92%           │
│          │     │     │     │     │     │                         │
│          ▼     ▼     ▼     ▼     ▼     ▼                         │
│  品質    4.8   5.5   6.3   7.0   7.8   8.8                      │
│          │     │     │     │     │     │                         │
│          ▼     ▼     ▼     ▼     ▼     ▼                         │
│  知見    45    150   450  1,000  2,200 4,200                     │
│                                                                  │
│  Phase:  A(高速BL) B(高速認識) C(精密予測) D(高精度運用)           │
│  学習/月: 900-2.4k  3k-6k      9k-45k     60k-105k              │
│  KPI: 平均Imp = プラットフォーム別平均インプレッション（≠アカウント数）│
│                                                                  │
├──────────────────────────────────────────────────────────────────┤
│  エージェント個別精度（12ヶ月後予測）                                │
│                                                                  │
...
1. **per-content学習で精度が急速に向上**: Month 1: 25-35% → Month 3: 45-60% → Month 6: 70-82% → Month 12: 88-93%（上限92%）。毎回のコンテンツ制作が学習イベントとなり、日次サイクル型の1,000〜3,000倍の学習速度を実現する

2. **上限は92%（楽観95%）**: per-contentモデルの圧倒的データ量（月間15,000〜105,000件）により、統計的にノイズを排除可能。残る8%はプラットフォームアルゴリズム変更、バイラル偶発性、季節変動等の予測不能な外部イベント。旧日次サイクル型（天井65〜70%）から+22〜27ptの向上
