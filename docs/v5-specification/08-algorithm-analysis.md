...
| 月 | リサーチャー | アナリスト | プランナー | 戦略エージェント | 備考 |
|---|---|---|---|---|---|
| 3月 | 40% | 20% | 15% | 25% | 全エージェント初期段階。一般知識ベース |
| 4月 | 48% | 28% | 20% | 30% | リサーチャーが最速で改善開始 |
| 5月 | 55% | 35% | 28% | 35% | アナリストのデータ蓄積が本格化 |
| 6月 | 60% | 42% | 35% | 40% | プランナーが品質パターンを学習 |
| 9月 | 68% | 55% | 45% | 50% | 全エージェントが実用精度に到達 |
| 12月 | 75% | 65% | 55% | 60% | 成熟段階。改善率は逓減 |
...
`agent_performance`テーブル（または既存テーブルへのカラム追加）で個別精度を追跡する:

| カラム | 型 | 説明 |
|---|---|---|
| `agent_id` | text | エージェントの識別子（e.g., `researcher-01`, `planner-beauty`） |
| `agent_type` | enum('researcher','analyst','planner','strategist') | エージェント種別 |
...
| `agent_performance`テーブルのスキーマ設計と実装 | Phase 1開発 | 3月 |
