...
```
新規アカウントのインプレッション成長フロー:

  [初期] 300-1,000 imp
    │
    ├─ Phase A: テストプール突破 (1-2ヶ月目)
    │   ├─ 学習: コホートベースラインで予測 (source='cohort')
    │   ├─ 効果: 完視聴率改善 → アルゴリズム評価上昇
    │   └─ 結果: 1,000-3,000 imp (3-10x成長)
    │
    ├─ Phase B: パターン最適化 (3-4ヶ月目)
    │   ├─ 学習: own_historyベースライン有効化 + 4-7補正係数活性化
    │   ├─ 効果: 投稿時間・フック形式・ハッシュタグの最適化
    │   └─ 結果: 3,000-8,000 imp (3-8x追加成長)
    │
    ├─ Phase C: 精密運用 (5-6ヶ月目)
    │   ├─ 学習: 全9補正係数活性化 + クロスニッチ知見転用
    │   ├─ 効果: コンテンツ品質スコア7+ + 最適投稿戦略
    │   └─ 結果: 8,000-15,000 imp (KPI目標到達圏)
    │
    └─ Phase D: 高精度維持 (7ヶ月目以降)
        ├─ 学習: 精緻な予測モデル + 外部変数即時対応
        ├─ 効果: 安定的にKPI目標を達成・超過
        └─ 結果: 15,000-30,000 imp (目標1-2x)
```
