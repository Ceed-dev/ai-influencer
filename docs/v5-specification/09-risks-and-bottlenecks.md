...
| **MCPツール数** | 89 MCPツール + 13 REST APIの設計・実装・テスト | 並列実装で分担可能だが、統合テストの工数は残る |
...
中-高│              R-20               R-07←(低減)
     │
  中 │ R-21  R-22  R-10  R-11  R-08←  R-09  R-15
     │              R-16
...
```
Tier 1 (即座)          Tier 2 (Week 1-3)        Tier 3 (Week 3-5)        Tier 4 (Week 5-7+)
┌──────────────┐      ┌──────────────────┐      ┌──────────────────┐      ┌──────────────┐
│ KPI修正       │─────→│ system_settings  │─────→│ コスト監視UI      │─────→│ 月次レビュー   │
│ スケール修正   │      │ 行動ログDB設計    │─────→│ 品質ゲート+レビュー│─────→│ 自動承認移行  │
│ AIラベル策定   │      │ Week毎動作保証   │      │ 戦略承認フロー    │      │ 統合テストバッファ│
│ API審査申請   │      │ LangGraph PoC    │─────→│ エラーリカバリー  │      │ バックアップ検証│
│ 仕様クロスレビュー│   │ 型定義ファイル    │─────→│ 早期結合テスト    │      │              │
│ スキーマ凍結   │      │ tool_catalog構築  │─────→│ レシピフォールバック│      │              │
