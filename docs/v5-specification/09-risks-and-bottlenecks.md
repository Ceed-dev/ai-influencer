...
| **MCPツール数** | 89 MCPツール + 13 REST APIの設計・実装・テスト | 単体テスト+結合テストの工数が膨大 |
...
|---|---|---|---|---|
| Phase 1 | データ基盤 | 3週間 | 低 | PostgreSQL/pgvectorは枯れた技術 |
| Phase 2 | ワーカー層 | 3週間 | 中 | v4.0コードの再利用で軽減 |
| Phase 3 | インテリジェンス層 | 4週間 | **高** | 新技術、LangGraph.js + Supervisorパターンの複雑さ |
| Phase 4 | 戦略・計画層 | 4週間 | **高** | Supervisorパターンの安定性、マルチPF対応 |
| Phase 5 | ダッシュボード + スケール | 5週間 | 中 | Next.js + Shadcn/uiは経験あり |
...
| リスク | 影響 | 緩和策 |
|--------|------|--------|
| 人間承認ボトルネック | REQUIRE_HUMAN_APPROVAL=true時に計画キューが溜まる | バッチ承認UI + 低リスク計画の自動承認ルール |
...
| ダッシュボード停止 | 自律システムへの人間の可視性がゼロに | PM2プロセス管理 + ヘルスチェックアラート |
...
| R-13 | PostgreSQL+pgvector運用 | 技術 | 低-中 | 低い | **C** | Cloud SQL (Phase 1から使用)、インスタンス監視 |
...
中-高│                   R-07
     │
  中 │ R-10  R-11  R-12  R-08  R-09  R-15
     │        R-16
低-中│ R-13  R-14  R-17
     │
  低 │ R-18
...
| 9 | **tool_catalogに代替ツール登録** | R-15, R-16 | ツールスペシャリストが参照するツールカタログの初期データ構築 |
| 10 | **Docker環境構築 (docker-compose.yml)** | R-17, R-18 | Phase 1でPostgreSQLコンテナから開始。dev/prod分離は環境変数のみ |

### Tier 3: Phase 3-4で実装

| # | 緩和策 | 対象リスク | 理由 |
|---|---|---|---|
| 11 | **品質ゲートの実装** | R-07 | コンテンツ品質スコアの閾値チェック |
| 12 | **人間承認フローの実装** | R-07 | 戦略変更時のダッシュボード承認UI |
| 13 | **コスト監視ダッシュボード** | R-08, R-12 | 日次コストの可視化とアラート |
| 14 | **コンテンツフィルター** | R-01, R-07 | 不適切コンテンツの自動検出 |
| 15 | **投稿タイミング分散ロジック** | R-04 | アカウント別ランダム投稿スケジュール |
| 16 | **ツールスペシャリストのデフォルトレシピフォールバック実装** | R-15 | 初期段階のツール選択ミスを防止。データ蓄積後に徐々に判断を委譲 |
| 17 | **production_recipesの品質チェックフロー** | R-16 | 新ツール組み合わせのテスト制作→品質評価→本番採用の承認フロー |

### Tier 4: Phase 5以降 (運用開始後)

| # | 緩和策 | 対象リスク | 理由 |
|---|---|---|---|
| 18 | **Cloud SQLインスタンスのスケールアップ検討** | R-13 | 3,500アカウント規模での性能に応じてインスタンスタイプ変更 |
| 19 | **マルチプロバイダー抽象化** | R-11 | API代替が必要になった時点で実装 |
| 20 | **デッドロック監視** | R-14 | 並行処理の問題が顕在化してから対応 |
| 21 | **月次コストレビュープロセス** | R-08 | 定常運用に入ってから導入 |
| 22 | **named volume定期バックアップの検証** | R-17 | 本番運用開始後にバックアップ・リストアの実地検証 |
...
│ スケール修正   │      │ 行動ログDB設計    │─────→│ 品質ゲート       │      │ バックアップ検証│
│ AIラベル策定   │      │ フェーズ毎動作保証 │      │ 人間承認フロー    │      │              │
│ API審査申請   │      │ LangGraph PoC    │─────→│ コンテンツフィルタ │      │              │
│              │      │ tool_catalog構築  │─────→│ レシピフォールバック│      │              │
│              │      │ Docker環境構築    │─────→│ 品質チェックフロー │      │              │
└──────────────┘      └──────────────────┘      └──────────────────┘      └──────────────┘
```
