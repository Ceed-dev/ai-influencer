...
export default function AgentsPage() {
  ...
  const getAgentLabel = (agentType: string) => {
    const key = `agents.agentLabels.${agentType}`;
    const value = t(key);
    ...
    <div>
      <Tabs defaultValue="thought-log">
        <TabsList className="mb-6">
          <TabsTrigger value="thought-log">{t("agents.tabs.thoughtLog")}</TabsTrigger>
          <TabsTrigger value="dialogue">{t("agents.tabs.dialogue")}</TabsTrigger>
          <TabsTrigger value="evolution">{t("agents.tabs.evolution")}</TabsTrigger>
          <TabsTrigger value="prompt">{t("agents.tabs.prompt")}</TabsTrigger>
          <TabsTrigger value="suggestions">{t("agents.tabs.suggestions")}</TabsTrigger>
          <TabsTrigger value="growth">{t("agents.tabs.growth")}</TabsTrigger>
          <TabsTrigger value="inbox">{t("agents.tabs.inbox")}</TabsTrigger>
        </TabsList>
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-2">
        <label htmlFor="agent-filter">{t("agents.agent")}</label>
        <NativeSelect
          id="agent-filter"
          className="w-auto"
          value={agentType}
          ...
          <CardContent className="pt-4">
            ...
            {expandedId === (log.id as string) && (
              <div className="mt-3 space-y-2 text-sm">
                <div>
                  <strong>{t("agents.inputData")}</strong>:{" "}
    ...
    <div className="flex gap-4">
      <div className="w-48 space-y-1">
        {AGENT_TYPES.map((at) => (
          <Button
            key={at}
            variant={selectedAgent === at ? "default" : "ghost"}
            size="sm"
      ...
      <div className="flex-1">
              ...
              <CardContent className="pt-3 pb-3">
                ...
                {msg.human_response ? (
                  <p className="mt-1 text-sm text-[var(--accent-cyan)]">
                    {t("agents.replyPrefix")} {msg.human_response as string}
                  </p>
                ) : null}
              </CardContent>
            </Card>
        ...
        <div className="flex gap-2">
          <Input
            placeholder={t("agents.enterInstruction")}
            value={newMessage}
            onChange={(e) => setNewMessage(e.target.value)}
            className="flex-1"
          />
          <NativeSelect
            aria-label={t("agents.priorityLabel")}
            className="w-auto"
            value={priority}
...
function EvolutionPanel() {
  ...
  for (const at of AGENT_TYPES) {
    ...
    for (const r of data) {
      ...
      existing[at] = r.self_score as number;
              ...
              <LineChart data={progressionData}>
                ...
                {activeAgents.map((at) => (
                  <Line
                    ...
                    type="monotone"
                    dataKey={at}
                    name={getAgentLabel(at)}
                    stroke={AGENT_COLORS[at] || COLORS.violet}
                    strokeWidth={2}
                    dot={{ r: 4 }}
                    connectNulls
                  />
            ...
            <CardContent className="pt-4">
              <div className="flex justify-between items-center">
                ...
                <Badge variant="secondary">
                  {data.length} {t("agents.reflections")}
                </Badge>
              </div>
              <div className="mt-2 text-sm space-y-1">
                <p>
                  {t("agents.latestSelfScore")}{" "}
                  <strong>{latest.self_score as number}</strong> (Cycle{" "}
                  {latest.cycle_id as number})
                </p>
                {(latest.what_went_well as string) ? (
                  <p className="text-muted-foreground">
                    {t("agents.whatWentWell")} {latest.what_went_well as string}
                  </p>
                ) : null}
                {(latest.what_to_improve as string) ? (
                  <p className="text-muted-foreground">
                    {t("agents.whatToImprove")} {latest.what_to_improve as string}
                  </p>
                ) : null}
              </div>
            </CardContent>
...
function PromptPanel() {
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-2">
        <label htmlFor="prompt-agent">{t("agents.agent")}</label>
        <NativeSelect
          id="prompt-agent"
          className="w-auto"
          value={agentType}
...
function GrowthPanel() {
  ...
  useState(() => {
    ...
    Promise.all([
      Promise.all(
        AGENT_TYPES.map((at) =>
          fetch(`/api/individual-learnings?agent_type=${at}&limit=100`)
            .then((res) => res.json())
            .then((d) => ({ agent_type: at, data: d.learnings || [] }))
        )
      ),
      fetch("/api/reflections?limit=200")
        .then((res) => res.json())
  ...
  const velocityData = AGENT_TYPES.map((at) => {
    const data = agentGroups[at] || [];
          ...
          <Card key={agent.agent_type as string} data-agent-card>
            <CardContent className="pt-4">
              <h3 className="font-semibold mb-2">
                {getAgentLabel(agent.agent_type as string)}
              </h3>
              <div className="space-y-1 text-sm">
                <p>
                  {t("agents.learningCount")}{" "}
                ...
                <p>
                  {t("agents.reflectionSuccessRate")}{" "}
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-4">
        <NativeSelect
          aria-label={t("production.status")}
          className="w-auto"
          value={statusFilter}
          onChange={(e) => {
      ...
      {messages.map((msg) => (
        <Card key={msg.id as string} data-message className="mb-2">
          <CardContent className="pt-3 pb-3">
            <div className="flex justify-between">
              <span className="font-semibold">
                {getAgentLabel(msg.agent_type as string)}
              </span>
              <Badge
                variant={
                  msg.status === "unread"
