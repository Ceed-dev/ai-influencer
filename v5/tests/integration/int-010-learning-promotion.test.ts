...
describe('FEAT-TST-010: individual learning → learnings promotion', () => {
  ...
  beforeAll(async () => {
    ...
    await client.query(`DELETE FROM agent_individual_learnings WHERE content LIKE '%INT010%'`);
  });
  ...
  test('TEST-INT-010: individual learning promoted after threshold reached', async () => {
    ...
    const indRes = await client.query(
      `INSERT INTO agent_individual_learnings (agent_type, category, content, times_applied, confidence)
       VALUES ('analyst', 'timing', 'INT010: Morning posts outperform evening by 30%', $1, 0.85)
       RETURNING id`,
      [threshold]
    );
    ...
    const indLearning = await client.query(
      `SELECT content, confidence FROM agent_individual_learnings WHERE id = $1 AND times_applied >= $2`,
    ...
    await client.query(
      `INSERT INTO learnings (category, insight, confidence, evidence_count)
       VALUES ('timing', $1, $2, $3)`,
      [indLearning.rows[0].content, indLearning.rows[0].confidence, threshold]
    );
